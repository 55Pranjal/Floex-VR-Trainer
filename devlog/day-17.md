# Day 17 Devlog — Floex VR Trainer
...
1. **Fix multi-canvas Meta XR registration** — write a runtime fixer script that forces per-instance registration in `PointableCanvasModule`, OR find an SDK-supported way to scope rays per canvas (Interactor groups / interaction layers).
2. **Once all 3 single pump heads interactive independently:** start the double pump head (4th slot) — different screen set, scoped when 3D model arrives.

## Carry-overs (unchanged from Day 16)
